...
class AssetGameSystem implements System {
    clear(): void {
        $.managerOpen = false;
        $.shortcuts = [];
        // A drag can't survive a session teardown, and leaving this set would
        // strand the board in its highlighted drop state.
        $.draggingToBoard = false;
    }

    setDraggingToBoard(dragging: boolean): void {
        $.draggingToBoard = dragging;
    }
