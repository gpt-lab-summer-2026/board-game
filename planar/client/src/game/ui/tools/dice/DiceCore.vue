...
// Derived from the expression rather than held separately, so it stays honest
// when the roll is typed by hand or recalled from history.
const rollBias = computed(() => {
    // Touch the parts so this recomputes as the expression is edited.
    void DxHelper.parts.value.length;
    void diceState.reactive.textInput;
    return DxHelper.getRollBias();
});

function toggleBias(bias: "advantage" | "disadvantage"): void {
    DxHelper.setRollBias(rollBias.value === bias ? "normal" : bias);
}
...
<template>
    <div id="dice-main" class="container">
        ...
        <!--
            Advantage / disadvantage. Both are toggles: pressing the active one
            returns the roll to a plain d20, which is what someone who clicked
            the wrong one wants. The resulting notation is shown so the
            shorthand stays learnable rather than magic.
        -->
        <div id="bias-selector">
            <span class="bias-label">{{ t("game.ui.tools.DiceTool.roll") }}</span>
            <button
                type="button"
                class="bias-button"
                :class="{ active: rollBias === 'disadvantage' }"
                :aria-pressed="rollBias === 'disadvantage'"
                title="Roll two d20 and keep the lowest (2d20kl1)"
                @click="toggleBias('disadvantage')"
            >
                Disadvantage
            </button>
            <button
                type="button"
                class="bias-button"
                :class="{ active: rollBias === 'advantage' }"
                :aria-pressed="rollBias === 'advantage'"
                title="Roll two d20 and keep the highest (2d20kh1)"
                @click="toggleBias('advantage')"
            >
                Advantage
            </button>
        </div>
...
#dice-main {
    ...
    #bias-selector {
        ...
        > .bias-button {
            ...
            /* Filled, not just tinted: this changes what the dice do, so it
               needs to be obvious at a glance that one is engaged. */
            &.active {
