...
import { ghostMenu } from "./contextmenu";
import { registerSpellHook } from "./spellhook";
...
async function initGame(gameApi: GameApi): Promise<void> {
    ...
    // Right-click a character token to level it up without opening the sheet.
    // A context menu entry is the only non-tab UI surface a mod gets, and it
    // is the one the request actually asked for.
    api.ui.shape.registerContextMenuEntry(levelUpEntry);

    // Spell templates announce who they caught; this mod owns what a
    // condition means, so it is the one that applies them.
    registerSpellHook();

    // Everything the ghost can do, on the token itself.
...
async function loadLocation(): Promise<void> {
    // Warm the catalogue here rather than only when a character tab is
    // opened. The right-click menu builds its condition submenus from it
    // synchronously, so without this the very first right-click on a fresh
    // session offers two empty lists.
