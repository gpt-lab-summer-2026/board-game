...
/**
 * Switch to a specific tool mode.
 *
 * Split out of `toggleActiveMode` so the UI can offer Build and Play as two
 * distinct choices rather than one control that flips. They *are* two choices
 * -- the mode decides which tools exist at all -- and a toggle cannot say which
 * one you are about to get.
 */
export function setActiveMode(mode: ToolMode): void {
    if (activeToolMode.value === mode) return;
    toggleActiveMode();
}
