...
import { snapPointToGrid, DEFAULT_HEX_RADIUS } from "../../../../core/grid";
...
export enum DrawShape {
    Square = "square",
    // The Line and Hex shape classes already existed and were simply never
...
class DrawTool extends Tool implements ITool {
    ...
    async onDown(lp: LocalPoint, event: MouseEvent | TouchEvent | undefined): Promise<void> {
        ...
        if (!this.active.value) {
            ...
            switch (this.state.selectedShape) {
                ...
                case DrawShape.Hex: {
                    // Sized in cells like the spell template, so a hex drawn
                    // here lands on the grid rather than near it.
                    this.shape = createHexPolygon(
                        cloneP(this.startPoint),
                        1,
                        {
                            type: locationSettingsState.raw.gridType.value,
                            oddHexOrientation: false,
                            radius: DEFAULT_HEX_RADIUS,
                        },
                        { lineWidth: [this.state.brushSize] },
                    );
                    // createHexPolygon builds the Polygon itself and takes no
                    // properties argument, so colours are applied after.
                    propertiesSystem.setFillColour(this.shape.id, this.colours.value.fill, NO_SYNC);
                    propertiesSystem.setStrokeColour(this.shape.id, this.colours.value.stroke, NO_SYNC);
                    break;
                }
    ...
    onMove(lp: LocalPoint, event: MouseEvent | TouchEvent | undefined): Promise<void> {
        ...
        switch (this.state.selectedShape) {
            ...
            case DrawShape.Line: {
                (this.shape as Line).endPoint = endPoint;
                break;
            }
            case DrawShape.Hex: {
                // Redraw at the new size: a hex polygon's vertices are
                // computed up front, so there is nothing to nudge.
                break;
            }
            case DrawShape.Brush: {
