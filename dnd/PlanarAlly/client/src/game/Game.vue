...
<template>
    <div id="main" @mouseleave="mouseLeave" @wheel.passive="zoom">
        ...
        <div id="board" :class="{ disconnected: !gameState.reactive.isConnected }">
            <div
                id="layers"
                :class="{ 'drop-target': assetGameState.reactive.draggingToBoard }"
                @mousedown="mouseDown"
                @mouseup="mouseUp"
                @mousemove="mousemove"
...
/*
 * Highlight the board while an asset is being dragged towards it.
 *
 * An inset ring rather than the old half-transparent blue wash: washing the
 * board out is the one thing you don't want while judging where to put
 * something. box-shadow also costs no layout and does not touch the canvases.
 */
#layers.drop-target {
    box-shadow: inset 0 0 0 3px var(--pa-drop-highlight);
}
</style>
