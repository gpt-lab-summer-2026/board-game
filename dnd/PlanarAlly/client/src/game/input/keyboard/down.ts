...
export async function onKeyDown(event: KeyboardEvent): Promise<void> {
    ...
    if (event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement) {
        ...
        if (navKeys.includes(event.code)) {
            ...
            uiSystem.toggleGhostConsole();
        } else if (event.key === "?") {
            // Every binding in this function is otherwise undiscoverable; see
            // keyboard/bindings.ts for the list the overlay renders.
            uiSystem.toggleShortcutHelp();
        } else {
            eventConsumed = false;
        }
    }
