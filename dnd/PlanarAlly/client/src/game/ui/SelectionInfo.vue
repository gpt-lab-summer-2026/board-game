...
#selection-menu {
    ...
    /*
     * This used to be `opacity: 0.5` on the whole panel, lifted to 1 on hover.
     * Container opacity dims the *text* along with the background, so a token's
     * name and HP were unreadable over a light map -- which is most of the time
     * someone actually wants to glance at them.
     *
     * Same intent, right lever: the panel recedes by letting the map show
     * through its background, while the type stays at full strength.
     */
    &:hover {
        z-index: 10;
    ...
    .info-notes {
        /* Was the CSS keyword `bisque`. */
        background-color: var(--pa-surface-note);
        padding: 0.5rem;
        display: flex;
        flex-direction: column;
