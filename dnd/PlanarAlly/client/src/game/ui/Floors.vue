...
<template>
    <div id="floor-layer">
        <!-- These were <a href="#"> wrapped in a clickable <div>: announced as
             links that go nowhere, and the accessible name disappeared entirely
             whenever tool icons were switched off. -->
        <div v-if="visible" id="floor-selector">
            <button
                type="button"
                :title="t('game.ui.FloorSelect.title')"
                :aria-label="t('game.ui.FloorSelect.title')"
                :aria-expanded="detailsOpen"
                @click.prevent="detailsOpen = !detailsOpen"
            >
                <template v-if="playerSettingsState.reactive.useToolIcons.value">
                    <img :src="getStaticFloorImg('floors.svg')" alt="" />
                </template>
        ...
        <div v-show="layers.length > 1" style="display: contents">
            <div v-for="layer in layers" :key="layer" class="layer">
                <button
                    ...
                    :title="layerTranslationMapping[layer]"
                    :aria-label="layerTranslationMapping[layer]"
                    :aria-pressed="layer === selectedLayer"
                    @click.prevent="selectLayer(layer)"
                >
                    <template v-if="playerSettingsState.reactive.useToolIcons.value">
                        <img :src="getStaticFloorImg(`${layer.toLowerCase()}.svg`)" alt="" />
                    </template>
...
.layer > button {
    ...
    /* The selected layer is the one thing here worth being sure about, so it
       gets a bar as well as the fill -- the accent green alone is ~1.9:1. */
    &.layer-selected {
        background-color: var(--pa-accent);
