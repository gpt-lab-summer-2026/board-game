...
AUDIO_DEVICE = int(os.getenv("AUDIO_DEVICE", 2)) # check correct device with 'python -m sounddevice' and set it in .env
FS = 48000
...
WAKE_WINDOW_SECONDS = 3.0  # only the most recent audio is scanned for the wake word
...
def record_audio():
    ...
    try:
        with sd.InputStream(samplerate=FS, channels=1, dtype="int16", device=AUDIO_DEVICE, callback=callback, latency="high") as stream:
            ...
            max_window_samples = int(WAKE_WINDOW_SECONDS * FS)
            while noWakeWord:
                print("listening for wake word")
                sd.sleep(POLL_INTERVAL_MS)
                # take only the frames captured so far; the callback keeps appending
                # past this point and those frames are picked up on the next tick
                ready = len(AUDIO_DATA)
                if not ready:
                    continue
                window = np.concatenate([window, *AUDIO_DATA[:ready]])
                del AUDIO_DATA[:ready]
                # drop anything older than the window so each tick costs the same
                window = window[-max_window_samples:]
                audio = resample(audio=window)
                ...
                if speech_timestamps:
                    ...
                    if detect_wake_word(audio=audio, timestamps=speech_timestamps):
                        ...
                        # said would otherwise be recorded back in as the command
                        stream.stop()
